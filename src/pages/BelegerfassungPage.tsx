...
export default function BelegerfassungPage() {
  const [records, setRecords] = useState<Belegerfassung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Belegerfassung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Belegerfassung | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Belegerfassung | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, belegpositionenData] = await Promise.all([
  ...
  async function handleCreate(fields: Belegerfassung['fields']) {
    await LivingAppsService.createBelegerfassungEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Belegerfassung['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateBelegerfassungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} Belegerfassung im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Belegerfassung suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('beleg_bemerkung')}>
                <span className="inline-flex items-center gap-1">
                  Bemerkungen zum Beleg
                  {sortKey === 'beleg_bemerkung' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('belegpositionen_uebersicht')}>
                <span className="inline-flex items-center gap-1">
                  Belegpositionen
                  {sortKey === 'belegpositionen_uebersicht' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.beleg_bemerkung ?? '—'}</span></TableCell>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getBelegpositionenDisplayName(record.fields.belegpositionen_uebersicht)}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={9} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Belegerfassung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
