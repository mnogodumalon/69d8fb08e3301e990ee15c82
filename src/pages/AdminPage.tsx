...
const USTABFUEHRUNGLEASINGFAHRZEUG_FIELDS = [
  { key: 'belegposition_auto', label: 'Belegposition automatisch erstellen', type: 'bool' },
  { key: 'belegposition_eigenanteil_referenz', label: 'Belegposition (Eigenanteil MwSt)', type: 'applookup/select', targetEntity: 'belegpositionen', targetAppId: 'BELEGPOSITIONEN', displayField: 'rechnungsnummer' },
...
const SKR03KONTENRAHMEN_FIELDS = [
  ...
  { key: 'kontenklasse', label: 'Kontenklasse', type: 'lookup/select', options: [{ key: 'klasse_0', label: 'Klasse 0 – Anlagevermögen' }, { key: 'klasse_1', label: 'Klasse 1 – Umlaufvermögen' }, { key: 'klasse_2', label: 'Klasse 2 – Eigenkapital' }, { key: 'klasse_3', label: 'Klasse 3 – Fremdkapital' }, { key: 'klasse_4', label: 'Klasse 4 – Betriebliche Aufwendungen' }, { key: 'klasse_5', label: 'Klasse 5 – Betriebliche Erträge' }, { key: 'klasse_6', label: 'Klasse 6 – Weitere Aufwendungen' }, { key: 'klasse_7', label: 'Klasse 7 – Weitere Erträge' }, { key: 'klasse_8', label: 'Klasse 8 – Abschlusskonten' }, { key: 'klasse_9', label: 'Klasse 9 – Vortragskonten' }] },
...
const KONTIERUNGUNDPRUEFUNG_FIELDS = [
  ...
  { key: 'plausibilitaet', label: 'Plausibilität', type: 'lookup/select', options: [{ key: 'plausibel', label: 'Plausibel' }, { key: 'pruefung_erforderlich', label: 'Prüfung erforderlich' }, { key: 'nicht_plausibel', label: 'Nicht plausibel' }, { key: 'nicht_geprueft', label: 'Nicht geprüft' }] },
...
const BELEGPOSITIONEN_FIELDS = [
  ...
  { key: 'ust_abfuehrung_referenz', label: 'UST-Abführung Leasingfahrzeug', type: 'applookup/select', targetEntity: 'ust_abfuehrung_leasingfahrzeug', targetAppId: 'UST_ABFUEHRUNG_LEASINGFAHRZEUG', displayField: 'buchungstext' },
  { key: 'eigenanteil_leasingmwst_flag', label: 'Eigenanteil MwSt Leasingfahrzeug', type: 'bool' },
...
const BELEGUEBERSICHT_FIELDS = [
  { key: 'beleg_liste', label: 'Belegliste', type: 'applookup/select', targetEntity: 'belegerfassung', targetAppId: 'BELEGERFASSUNG', displayField: 'beleg_bemerkung' },
  { key: 'positionen_liste', label: 'Belegpositionen zum Beleg', type: 'applookup/select', targetEntity: 'belegpositionen', targetAppId: 'BELEGPOSITIONEN', displayField: 'rechnungsnummer' },
];
...
const ENTITY_TABS = [
  ...
  { key: 'beleguebersicht', label: 'Belegübersicht', pascal: 'Beleguebersicht' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('belegerfassung');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'beleguebersicht': return (data as any).beleguebersicht as Beleguebersicht[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      ...
      case 'belegpositionen':
        lists.ust_abfuehrung_leasingfahrzeugList = (data as any).ustAbfuehrungLeasingfahrzeug ?? [];
        lists.belegerfassungList = (data as any).belegerfassung ?? [];
        break;
      case 'beleguebersicht':
        lists.belegerfassungList = (data as any).belegerfassung ?? [];
        lists.belegpositionenList = (data as any).belegpositionen ?? [];
        break;
    }
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'ust_abfuehrung_leasingfahrzeug' && fieldKey === 'belegposition_eigenanteil_referenz') {
      const match = (lists.belegpositionenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.rechnungsnummer ?? '—';
    }
    ...
    if (entity === 'belegpositionen' && fieldKey === 'ust_abfuehrung_referenz') {
      const match = (lists.ust_abfuehrung_leasingfahrzeugList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.buchungstext ?? '—';
    }
    ...
    if (entity === 'beleguebersicht' && fieldKey === 'beleg_liste') {
      const match = (lists.belegerfassungList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.beleg_bemerkung ?? '—';
    }
    if (entity === 'beleguebersicht' && fieldKey === 'positionen_liste') {
      const match = (lists.belegpositionenList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.rechnungsnummer ?? '—';
    }
    return String(url);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'beleguebersicht': return BELEGUEBERSICHT_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'beleguebersicht': return {
        create: (fields: any) => LivingAppsService.createBeleguebersichtEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateBeleguebersichtEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteBeleguebersichtEntry(id),
      };
      default: return null;
    }
  }, []);
    ...
    >
      ...
      {(createEntity === 'belegpositionen' || dialogState?.entity === 'belegpositionen') && (
        <BelegpositionenDialog
          ...
          ust_abfuehrung_leasingfahrzeugList={(data as any).ustAbfuehrungLeasingfahrzeug ?? []}
      ...
      {(createEntity === 'beleguebersicht' || dialogState?.entity === 'beleguebersicht') && (
        <BeleguebersichtDialog
          open={createEntity === 'beleguebersicht' || dialogState?.entity === 'beleguebersicht'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'beleguebersicht' ? handleUpdate : (fields: any) => handleCreate('beleguebersicht', fields)}
          defaultValues={dialogState?.entity === 'beleguebersicht' ? dialogState.record?.fields : undefined}
          belegerfassungList={(data as any).belegerfassung ?? []}
          belegpositionenList={(data as any).belegpositionen ?? []}
      ...
      {viewState?.entity === 'beleguebersicht' && (
        <BeleguebersichtViewDialog
          open={viewState?.entity === 'beleguebersicht'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'beleguebersicht', record: r }); }}
          belegerfassungList={(data as any).belegerfassung ?? []}
          belegpositionenList={(data as any).belegpositionen ?? []}
        />
      )}
