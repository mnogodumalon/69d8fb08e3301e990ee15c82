...
import type { Belegerfassung, UstAbfuehrungLeasingfahrzeug, ExportUndAusgabe, Skr03Kontenrahmen, Leasingfahrzeug, KontierungUndPruefung, Belegpositionen, Beleguebersicht, CreateBelegerfassung, CreateUstAbfuehrungLeasingfahrzeug, CreateExportUndAusgabe, CreateSkr03Kontenrahmen, CreateLeasingfahrzeug, CreateKontierungUndPruefung, CreateBelegpositionen, CreateBeleguebersicht } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getUstAbfuehrungLeasingfahrzeug(): Promise<UstAbfuehrungLeasingfahrzeug[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.UST_ABFUEHRUNG_LEASINGFAHRZEUG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as UstAbfuehrungLeasingfahrzeug[];
    return enrichLookupFields(records, 'ust_abfuehrung_leasingfahrzeug');
  }
  static async getUstAbfuehrungLeasingfahrzeugEntry(id: string): Promise<UstAbfuehrungLeasingfahrzeug | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.UST_ABFUEHRUNG_LEASINGFAHRZEUG}/records/${id}`);
    const record = { record_id: data.id, ...data } as UstAbfuehrungLeasingfahrzeug;
    return enrichLookupFields([record], 'ust_abfuehrung_leasingfahrzeug')[0];
  }
  static async createUstAbfuehrungLeasingfahrzeugEntry(fields: CreateUstAbfuehrungLeasingfahrzeug) {
    return callApi('POST', `/apps/${APP_IDS.UST_ABFUEHRUNG_LEASINGFAHRZEUG}/records`, { fields: cleanFieldsForApi(fields as any, 'ust_abfuehrung_leasingfahrzeug') });
  }
  static async updateUstAbfuehrungLeasingfahrzeugEntry(id: string, fields: Partial<CreateUstAbfuehrungLeasingfahrzeug>) {
    return callApi('PATCH', `/apps/${APP_IDS.UST_ABFUEHRUNG_LEASINGFAHRZEUG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'ust_abfuehrung_leasingfahrzeug') });
  }
  static async deleteUstAbfuehrungLeasingfahrzeugEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.UST_ABFUEHRUNG_LEASINGFAHRZEUG}/records/${id}`);
  }
  ...
  // --- LEASINGFAHRZEUG ---
  static async getLeasingfahrzeug(): Promise<Leasingfahrzeug[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.LEASINGFAHRZEUG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Leasingfahrzeug[];
    return enrichLookupFields(records, 'leasingfahrzeug');
  }
  static async getLeasingfahrzeugEntry(id: string): Promise<Leasingfahrzeug | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.LEASINGFAHRZEUG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Leasingfahrzeug;
    return enrichLookupFields([record], 'leasingfahrzeug')[0];
  }
  static async createLeasingfahrzeugEntry(fields: CreateLeasingfahrzeug) {
    return callApi('POST', `/apps/${APP_IDS.LEASINGFAHRZEUG}/records`, { fields: cleanFieldsForApi(fields as any, 'leasingfahrzeug') });
  }
  static async updateLeasingfahrzeugEntry(id: string, fields: Partial<CreateLeasingfahrzeug>) {
    return callApi('PATCH', `/apps/${APP_IDS.LEASINGFAHRZEUG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'leasingfahrzeug') });
  }
  static async deleteLeasingfahrzeugEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.LEASINGFAHRZEUG}/records/${id}`);
  }
  ...
  // --- BELEGUEBERSICHT ---
  static async getBeleguebersicht(): Promise<Beleguebersicht[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BELEGUEBERSICHT}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Beleguebersicht[];
    return enrichLookupFields(records, 'beleguebersicht');
  }
  static async getBeleguebersichtEntry(id: string): Promise<Beleguebersicht | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BELEGUEBERSICHT}/records/${id}`);
    const record = { record_id: data.id, ...data } as Beleguebersicht;
    return enrichLookupFields([record], 'beleguebersicht')[0];
  }
  static async createBeleguebersichtEntry(fields: CreateBeleguebersicht) {
    return callApi('POST', `/apps/${APP_IDS.BELEGUEBERSICHT}/records`, { fields: cleanFieldsForApi(fields as any, 'beleguebersicht') });
  }
  static async updateBeleguebersichtEntry(id: string, fields: Partial<CreateBeleguebersicht>) {
    return callApi('PATCH', `/apps/${APP_IDS.BELEGUEBERSICHT}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'beleguebersicht') });
  }
  static async deleteBeleguebersichtEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BELEGUEBERSICHT}/records/${id}`);
  }
