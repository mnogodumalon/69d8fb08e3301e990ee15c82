...
export interface Belegerfassung {
  ...
  fields: {
    ...
    beleg_datei?: string;
    belegpositionen_uebersicht?: string; // applookup -> URL zu 'Belegpositionen' Record
  };
}

export interface UstAbfuehrungLeasingfahrzeug {
  record_id: string;
  createdat: string;
  updatedat: string | null;
  fields: {
    belegposition_auto?: boolean;
    belegposition_eigenanteil_referenz?: string; // applookup -> URL zu 'Belegpositionen' Record
    fahrzeug_referenz?: string; // applookup -> URL zu 'Leasingfahrzeug' Record
    ust_zeitraum?: string; // Format: YYYY-MM-DD oder ISO String
    privatnutzung_betrag?: number;
    bemessungsgrundlage_netto?: number;
    ust_satz?: LookupValue;
    ust_betrag?: number;
    buchungstext?: string;
    belegposition_referenz?: string; // applookup -> URL zu 'Belegpositionen' Record
    buchungsstatus?: LookupValue;
    ust_bemerkung?: string;
  };
}
...
export interface Leasingfahrzeug {
  record_id: string;
  createdat: string;
  updatedat: string | null;
  fields: {
    ...
    nutzungsart?: LookupValue;
    skr03_konto_leasing?: string; // applookup -> URL zu 'Skr03Kontenrahmen' Record
    skr03_konto_ust?: string; // applookup -> URL zu 'Skr03Kontenrahmen' Record
  };
}
...
export interface Belegpositionen {
  ...
  fields: {
    ...
    ust_abfuehrung_referenz?: string; // applookup -> URL zu 'UstAbfuehrungLeasingfahrzeug' Record
    eigenanteil_leasingmwst_flag?: boolean;
...
export interface Beleguebersicht {
  record_id: string;
  createdat: string;
  updatedat: string | null;
  fields: {
    beleg_liste?: string; // applookup -> URL zu 'Belegerfassung' Record
    positionen_liste?: string; // applookup -> URL zu 'Belegpositionen' Record
  };
}

export const APP_IDS = {
  BELEGERFASSUNG: '69d8fae8666f4fa5ddd1a8b6',
  UST_ABFUEHRUNG_LEASINGFAHRZEUG: '69db65a253c3e15463e34826',
  EXPORT_UND_AUSGABE: '69d8faea03592afd38c20888',
  SKR03_KONTENRAHMEN: '69d8fae09a27734ee7faa252',
  LEASINGFAHRZEUG: '69db659e5fd46be27d40b8da',
  KONTIERUNG_UND_PRUEFUNG: '69d8faea4e6ba5c11bf424fd',
  BELEGPOSITIONEN: '69d8fae8bbe0c2d0fb5178fa',
  BELEGUEBERSICHT: '69db9256646d543fefb5a213',
} as const;
...
export const LOOKUP_OPTIONS: Record<string, Record<string, {key: string, label: string}[]>> = {
  ...
  'ust_abfuehrung_leasingfahrzeug': {
    ust_satz: [{ key: "ust_19", label: "19 %" }, { key: "ust_7", label: "7 %" }],
    buchungsstatus: [{ key: "offen", label: "Offen" }, { key: "gebucht", label: "Gebucht" }, { key: "exportiert", label: "Exportiert" }],
  },
  'export_und_ausgabe': {
  ...
  'leasingfahrzeug': {
    nutzungsart: [{ key: "fahrtenbuch", label: "Fahrtenbuch" }, { key: "ein_prozent_regel", label: "1%-Regel" }],
  },
  'kontierung_und_pruefung': {
    plausibilitaet: [{ key: "plausibel", label: "Plausibel" }, { key: "pruefung_erforderlich", label: "Prüfung erforderlich" }, { key: "nicht_plausibel", label: "Nicht plausibel" }, { key: "nicht_geprueft", label: "Nicht geprüft" }],
  },
  'belegpositionen': {
...
export const FIELD_TYPES: Record<string, Record<string, string>> = {
  'belegerfassung': {
    ...
    'beleg_datei': 'file',
    'belegpositionen_uebersicht': 'applookup/select',
  },
  'ust_abfuehrung_leasingfahrzeug': {
    'belegposition_auto': 'bool',
    'belegposition_eigenanteil_referenz': 'applookup/select',
    'fahrzeug_referenz': 'applookup/select',
    'ust_zeitraum': 'date/date',
    'privatnutzung_betrag': 'number',
    'bemessungsgrundlage_netto': 'number',
    'ust_satz': 'lookup/select',
    'ust_betrag': 'number',
    'buchungstext': 'string/textarea',
    'belegposition_referenz': 'applookup/select',
    'buchungsstatus': 'lookup/select',
    'ust_bemerkung': 'string/textarea',
  },
  'export_und_ausgabe': {
  ...
  'leasingfahrzeug': {
    ...
    'leasingvertrag_nummer': 'string/text',
    'leasingrate_brutto': 'number',
    'leasingbeginn': 'date/date',
    'leasingende': 'date/date',
    'listenpreis_brutto': 'number',
    'nutzungsart': 'lookup/select',
    'skr03_konto_leasing': 'applookup/select',
    'skr03_konto_ust': 'applookup/select',
  },
  'kontierung_und_pruefung': {
  ...
  'belegpositionen': {
    ...
    'ust_abfuehrung_referenz': 'applookup/select',
    'eigenanteil_leasingmwst_flag': 'bool',
  ...
  'beleguebersicht': {
    'beleg_liste': 'applookup/select',
    'positionen_liste': 'applookup/select',
  },
};
...
export type CreateUstAbfuehrungLeasingfahrzeug = StripLookup<UstAbfuehrungLeasingfahrzeug['fields']>;
...
export type CreateLeasingfahrzeug = StripLookup<Leasingfahrzeug['fields']>;
...
export type CreateBeleguebersicht = StripLookup<Beleguebersicht['fields']>;