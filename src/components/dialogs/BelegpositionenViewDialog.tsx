...
export function BelegpositionenViewDialog({ open, onClose, record, onEdit, ust_abfuehrung_leasingfahrzeugList, belegerfassungList }: BelegpositionenViewDialogProps) {
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">UST-Abführung Leasingfahrzeug</Label>
            <p className="text-sm">{getUstAbfuehrungLeasingfahrzeugDisplayName(record.fields.ust_abfuehrung_referenz)}</p>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Eigenanteil MwSt Leasingfahrzeug</Label>
            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${
              record.fields.eigenanteil_leasingmwst_flag ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'
            }`}>
              {record.fields.eigenanteil_leasingmwst_flag ? 'Ja' : 'Nein'}
            </span>
          </div>
          <div className="space-y-1">
