...
export function UstAbfuehrungLeasingfahrzeugViewDialog({ open, onClose, record, onEdit, belegpositionenList, leasingfahrzeugList }: UstAbfuehrungLeasingfahrzeugViewDialogProps) {
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Belegposition automatisch erstellen</Label>
            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${
              record.fields.belegposition_auto ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'
            }`}>
              {record.fields.belegposition_auto ? 'Ja' : 'Nein'}
            </span>
          </div>
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Belegposition (Eigenanteil MwSt)</Label>
            <p className="text-sm">{getBelegpositionenDisplayName(record.fields.belegposition_eigenanteil_referenz)}</p>
          </div>
          <div className="space-y-1">
