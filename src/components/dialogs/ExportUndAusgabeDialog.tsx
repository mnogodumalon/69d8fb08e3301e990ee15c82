...
interface ExportUndAusgabeDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: ExportUndAusgabe['fields']) => Promise<void>;
  defaultValues?: ExportUndAusgabe['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function ExportUndAusgabeDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: ExportUndAusgabeDialogProps) {
  const [fields, setFields] = useState<Partial<ExportUndAusgabe['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'export_und_ausgabe');
      await onSubmit(clean as ExportUndAusgabe['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "export_bezeichnung": string | null, // Exportbezeichnung\n  "zeitraum_von": string | null, // YYYY-MM-DD\n  "zeitraum_bis": string | null, // YYYY-MM-DD\n  "exportformat": LookupValue[] | null, // Exportformat (select one or more keys: "csv" | "elster_extf") mapping: csv=CSV-Datei, elster_extf=ELSTER / DATEV EXTF\n  "exportstatus": LookupValue | null, // Exportstatus (select one key: "ausstehend" | "in_bearbeitung" | "abgeschlossen" | "fehler") mapping: ausstehend=Ausstehend, in_bearbeitung=In Bearbeitung, abgeschlossen=Abgeschlossen, fehler=Fehler\n  "exportdatum": string | null, // YYYY-MM-DDTHH:MM\n  "dateiname": string | null, // Dateiname\n  "export_bemerkung": string | null, // Bemerkungen zum Export\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<ExportUndAusgabe['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Export und Ausgabe bearbeiten' : 'Export und Ausgabe hinzufügen';
