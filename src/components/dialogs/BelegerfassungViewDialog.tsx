...
export function BelegerfassungViewDialog({ open, onClose, record, onEdit, belegpositionenList }: BelegerfassungViewDialogProps) {
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <Label className="text-xs text-muted-foreground">Belegpositionen</Label>
            <p className="text-sm">{getBelegpositionenDisplayName(record.fields.belegpositionen_uebersicht)}</p>
          </div>
        </div>
      </DialogContent>
    </Dialog>
