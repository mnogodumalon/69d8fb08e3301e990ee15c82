...
interface BelegerfassungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Belegerfassung['fields']) => Promise<void>;
  defaultValues?: Belegerfassung['fields'];
  belegpositionenList: Belegpositionen[];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function BelegerfassungDialog({ open, onClose, onSubmit, defaultValues, belegpositionenList, enablePhotoScan = true, enablePhotoLocation = true }: BelegerfassungDialogProps) {
  const [fields, setFields] = useState<Partial<Belegerfassung['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'belegerfassung');
      await onSubmit(clean as Belegerfassung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="belegpositionen_uebersicht" entity="Belegpositionen">\n${JSON.stringify(belegpositionenList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "belegtyp": LookupValue | null, // Belegtyp (select one key: "eingangsrechnung" | "ausgangsrechnung" | "gutschrift" | "kassenbon" | "reisekostenbeleg" | "sonstiger_beleg") mapping: eingangsrechnung=Eingangsrechnung, ausgangsrechnung=Ausgangsrechnung, gutschrift=Gutschrift, kassenbon=Kassenbon, reisekostenbeleg=Reisekostenbeleg, sonstiger_beleg=Sonstiger Beleg\n  "dokumentklassifikation": LookupValue | null, // Dokumentklassifikation (select one key: "rechnung" | "gutschrift_klass" | "mahnung" | "lieferschein" | "vertrag" | "sonstiges") mapping: rechnung=Rechnung, gutschrift_klass=Gutschrift, mahnung=Mahnung, lieferschein=Lieferschein, vertrag=Vertrag, sonstiges=Sonstiges\n  "ocr_status": LookupValue | null, // OCR-Status (select one key: "ausstehend" | "in_verarbeitung" | "abgeschlossen" | "fehler") mapping: ausstehend=Ausstehend, in_verarbeitung=In Verarbeitung, abgeschlossen=Abgeschlossen, fehler=Fehler\n  "verarbeitungsstatus": LookupValue | null, // Verarbeitungsstatus (select one key: "neu" | "in_bearbeitung" | "geprueft" | "freigegeben" | "abgelehnt") mapping: neu=Neu, in_bearbeitung=In Bearbeitung, geprueft=Geprüft, freigegeben=Freigegeben, abgelehnt=Abgelehnt\n  "upload_datum": string | null, // YYYY-MM-DDTHH:MM\n  "beleg_bemerkung": string | null, // Bemerkungen zum Beleg\n  "belegpositionen_uebersicht": string | null, // Display name from Belegpositionen (see <available-records>)\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["belegpositionen_uebersicht"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const belegpositionen_uebersichtName = raw['belegpositionen_uebersicht'] as string | null;
        if (belegpositionen_uebersichtName) {
          const belegpositionen_uebersichtMatch = belegpositionenList.find(r => matchName(belegpositionen_uebersichtName!, [String(r.fields.rechnungsnummer ?? '')]));
          if (belegpositionen_uebersichtMatch) merged['belegpositionen_uebersicht'] = createRecordUrl(APP_IDS.BELEGPOSITIONEN, belegpositionen_uebersichtMatch.record_id);
        }
        return merged as Partial<Belegerfassung['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues ? 'Belegerfassung bearbeiten' : 'Belegerfassung hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="belegpositionen_uebersicht">Belegpositionen</Label>
            <Select
              value={extractRecordId(fields.belegpositionen_uebersicht) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, belegpositionen_uebersicht: v === 'none' ? undefined : createRecordUrl(APP_IDS.BELEGPOSITIONEN, v) }))}
            >
              <SelectTrigger id="belegpositionen_uebersicht"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
