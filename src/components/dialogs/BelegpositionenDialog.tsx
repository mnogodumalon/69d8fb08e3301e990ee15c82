...
interface BelegpositionenDialogProps {
  ...
  ust_abfuehrung_leasingfahrzeugList: UstAbfuehrungLeasingfahrzeug[];
  belegerfassungList: Belegerfassung[];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function BelegpositionenDialog({ open, onClose, onSubmit, defaultValues, ust_abfuehrung_leasingfahrzeugList, belegerfassungList, enablePhotoScan = true, enablePhotoLocation = true }: BelegpositionenDialogProps) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="ust_abfuehrung_referenz" entity="UST-Abführung Leasingfahrzeug">\n${JSON.stringify(ust_abfuehrung_leasingfahrzeugList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["ust_abfuehrung_referenz", "beleg_referenz"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const ust_abfuehrung_referenzName = raw['ust_abfuehrung_referenz'] as string | null;
        if (ust_abfuehrung_referenzName) {
          const ust_abfuehrung_referenzMatch = ust_abfuehrung_leasingfahrzeugList.find(r => matchName(ust_abfuehrung_referenzName!, [String(r.fields.buchungstext ?? '')]));
          if (ust_abfuehrung_referenzMatch) merged['ust_abfuehrung_referenz'] = createRecordUrl(APP_IDS.UST_ABFUEHRUNG_LEASINGFAHRZEUG, ust_abfuehrung_referenzMatch.record_id);
        }
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="ust_abfuehrung_referenz">UST-Abführung Leasingfahrzeug</Label>
            <Select
              value={extractRecordId(fields.ust_abfuehrung_referenz) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, ust_abfuehrung_referenz: v === 'none' ? undefined : createRecordUrl(APP_IDS.UST_ABFUEHRUNG_LEASINGFAHRZEUG, v) }))}
            >
              <SelectTrigger id="ust_abfuehrung_referenz"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                {ust_abfuehrung_leasingfahrzeugList.map(r => (
                  <SelectItem key={r.record_id} value={r.record_id}>
                    {r.fields.buchungstext ?? r.record_id}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
          ...
          <div className="space-y-2">
            <Label htmlFor="eigenanteil_leasingmwst_flag">Eigenanteil MwSt Leasingfahrzeug</Label>
            <div className="flex items-center gap-2 pt-1">
              <Checkbox
                id="eigenanteil_leasingmwst_flag"
                checked={!!fields.eigenanteil_leasingmwst_flag}
                onCheckedChange={(v) => setFields(f => ({ ...f, eigenanteil_leasingmwst_flag: !!v }))}
              />
              <Label htmlFor="eigenanteil_leasingmwst_flag" className="font-normal">Eigenanteil MwSt Leasingfahrzeug</Label>
            </div>
          </div>
          <div className="space-y-2">
