import type { EnrichedBelegerfassung, EnrichedBelegpositionen, EnrichedBeleguebersicht, EnrichedKontierungUndPruefung, EnrichedLeasingfahrzeug, EnrichedUstAbfuehrungLeasingfahrzeug } from '@/types/enriched';
import type { Belegerfassung, Belegpositionen, Beleguebersicht, KontierungUndPruefung, Leasingfahrzeug, Skr03Kontenrahmen, UstAbfuehrungLeasingfahrzeug } from '@/types/app';
...
export function enrichUstAbfuehrungLeasingfahrzeug(
  ...
  return ustAbfuehrungLeasingfahrzeug.map(r => ({
    ...r,
    belegposition_eigenanteil_referenzName: resolveDisplay(r.fields.belegposition_eigenanteil_referenz, maps.belegpositionenMap, 'rechnungsnummer'),
...
interface BelegpositionenMaps {
  ustAbfuehrungLeasingfahrzeugMap: Map<string, UstAbfuehrungLeasingfahrzeug>;
...
export function enrichBelegpositionen(
  ...
  return belegpositionen.map(r => ({
    ...r,
    ust_abfuehrung_referenzName: resolveDisplay(r.fields.ust_abfuehrung_referenz, maps.ustAbfuehrungLeasingfahrzeugMap, 'buchungstext'),
    beleg_referenzName: resolveDisplay(r.fields.beleg_referenz, maps.belegerfassungMap, 'beleg_bemerkung'),
  }));
}

interface BeleguebersichtMaps {
  belegerfassungMap: Map<string, Belegerfassung>;
  belegpositionenMap: Map<string, Belegpositionen>;
}

export function enrichBeleguebersicht(
  beleguebersicht: Beleguebersicht[],
  maps: BeleguebersichtMaps
): EnrichedBeleguebersicht[] {
  return beleguebersicht.map(r => ({
    ...r,
    beleg_listeName: resolveDisplay(r.fields.beleg_liste, maps.belegerfassungMap, 'beleg_bemerkung'),
    positionen_listeName: resolveDisplay(r.fields.positionen_liste, maps.belegpositionenMap, 'rechnungsnummer'),
  }));
}
